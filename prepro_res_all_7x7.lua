...
for i=1,sz,batch_size do
    ...
    net:forward(ims)
    feat7x7=net:get(8):get(3):get(2).output:clone()
    feat1x1=net:get(10).output:clone()
    if opt.l2norm then
        local batch_size=r-i+1
        local l2normalizer=nn.Sequential()
            :add(nn.Transpose({2,3},{3,4}))
            :add(nn.Reshape(batch_size*7*7,2048,false))
            :add(nn.Normalize(2))
            :add(nn.Reshape(batch_size,7,7,2048,false))
            :add(nn.Transpose({3,4},{2,3}))
        l2normalizer=l2normalizer:cuda()
        feat7x7=l2normalizer:forward(feat7x7)
        feat1x1=l2normalizer1x1:forward(feat1x1)
    end
    for j=1,r-i+1 do
