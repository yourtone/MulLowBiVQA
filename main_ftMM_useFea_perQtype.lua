...
input_ques_h5_pertype = paths.concat(input_path, 'data_pertype.h5')
input_json_pertype = paths.concat(input_path, 'data_pertype.json')
------ path setting end ------
...
print('DataLoader loading pertype json file: ', input_json_pertype)
local f = io.open(input_json_pertype, 'r')
local text = f:read()
f:close()
QT = cjson.decode(text)
...
print('DataLoader loading pertype h5 file: ', input_ques_h5_pertype)
local h5f = hdf5.open(input_ques_h5_pertype, 'r')
assert(torch.sum(torch.eq(trainset['ques_id'],h5f:read('/qid_train'):all()))==trainset['ques_id']:size(1))
assert(torch.sum(torch.eq(testset['ques_id'],h5f:read('/qid_test'):all()))==testset['ques_id']:size(1))
trainset['ques_type'] = h5f:read('/qtype_train'):all()
testset['ques_type'] = h5f:read('/qtype_test'):all()
h5f:close()
...
local qinds=torch.eq(testset['ques_type'],qtype):nonzero():view(-1)
testset['question'] = testset['question']:index(1,qinds)
testset['ques_id'] = testset['ques_id']:index(1,qinds)
testset['lengths_q'] = testset['lengths_q']:index(1,qinds)
testset['img_list'] = testset['img_list']:index(1,qinds)
trainset.N = trainset['question']:size(1)
testset.N = testset['question']:size(1)
...
function test(model_append)
   ...
   collectgarbage()
   os.execute(string.format('python vqaEval_v2.py --resultDir %s --methodInfo %s \z
      --nolenassert True --inputqidlist True > /dev/null', 
...
for iter = opt.previous_iters + 1, opt.max_iters do
   ...
   if trainset.ep > epoch then -- finished one epoch
      ...
      -- do evaluation
      timer:reset();
      local acc = test(string.format('_ep%d',epoch))
